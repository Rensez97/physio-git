...
def get_website(name, place):
    try:
        ...
        base1 = "https://places.googleapis.com/v1/places:searchText"
        payload = {
        ...
        response = requests.post(base1, json=payload, headers=headers)
        place_id = response.json()['places'][0]['id']

        base2 = f"https://places.googleapis.com/v1/places{place_id}"
        headers = {
        'Content-Type': 'application/json',
        'X-Goog-Api-Key': key,
        ...
        response = requests.post(base1, json=payload, headers=headers)
        website = response.json()['places'][0]['websiteUri']
        print("MAPS:",website)
    except:
...
#         places = gmaps.places(f'{name} {place}', language='en')
#         place_id = places['results'][0]['place_id']
#         print(place_id)
#         place_details = gmaps.place(place_id,fields='website')
#         print(place_details)
